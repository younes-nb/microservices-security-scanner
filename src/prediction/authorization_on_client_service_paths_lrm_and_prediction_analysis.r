...
training_file <- "../../data/authorization_on_client_service_paths.csv"
prediction_file <- "../../output/stats/authorization_on_client_service_paths.csv"

if (!file.exists(prediction_file) || file.info(prediction_file)$size == 0) {
  cat("[Client-Path Authorization] Prediction failed: No input data found.\n")
} else {
  ...
  required_cols <- c("AUC_T", "AUC_E", "AUC_P", "AUC_C")
  if (all(required_cols %in% colnames(prediction_data))) {
    training_data$GT <- factor(training_data$GT, levels = c("0", "1", "2", "3", "4"), ordered = TRUE)
    lrm_model <- lrm(GT ~ AUC_T + AUC_E + AUC_P + AUC_C, data = training_data, x = TRUE, y = TRUE, maxit=1000)
    ...
    final_output <- paste("[Client-Path Authorization] Predicted Security Level:", friendly_message)
    cat(final_output)
  } else {
    missing_cols <- paste(setdiff(required_cols, colnames(prediction_data)), collapse=", ")
    cat(paste("[Client-Path Authorization] Prediction failed: Input data is missing required columns:", missing_cols, "\n"))
  }
}